...
def tag_callback(msg):
    if len(msg.detections) > 0:
        ...
        for detection in msg.detections:
            tag_id = detection.id[0]  # Get the first detected tag ID
            pose = detection.pose.pose.pose  # Extract pose
            
            rospy.loginfo(f"Detected Tag ID: {tag_id}")
            rospy.loginfo(f"Position: x={pose.position.x}, y={pose.position.y}, z={pose.position.z}")
            
            # Publish transform for visualization in RViz
            transform = TransformStamped()
            transform.header.stamp = rospy.Time.now()
            transform.header.frame_id = "D435i_camera_color_optical_frame"  # Adjust based on your setup
...
detection_sub = rospy.Subscriber("/tag_detections", AprilTagDetectionArray, tag_callback)
